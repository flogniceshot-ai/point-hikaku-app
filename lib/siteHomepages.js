...
// オファーの申し込みリンク先を優先順位に沿って1つ決める(CampaignCard/OfferRow共通)。
// 優先順位: ①計測付きディープリンク → ②自社発行のアフィリエイトURL →
// ③(計測は無いが)そのメディア自身のドメインの案件詳細ページ → ④サイト本体トップページ →
// ⑤出典元URL(引用ブログ等)。
// ③をSITE_HOMEPAGESより先に見るのがポイント: sourceUrlがそのサイト自身の案件詳細ページなら、
// 計測が無くても一般的なトップページより具体的で役に立つため。
export function getBestOfferUrl(offer) {
  if (!offer) return null;
  const ownSourceUrl = isSiteOwnUrl(offer.sourceUrl, offer.siteSlug) ? offer.sourceUrl : null;
  return (
    buildDeepAffiliateUrl(offer) ||
    offer.mediaAffiliateUrl ||
    ownSourceUrl ||
    SITE_HOMEPAGES[offer.siteSlug] ||
    offer.sourceUrl ||
    null
  );
}
