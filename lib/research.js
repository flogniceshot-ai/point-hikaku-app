...
// offers[].valueは常に「サイト上の生のポイント数(または%)」で保持する
// (円換算はアプリ側のpoint_sites.point_rateで別途行うため)。
...
【重要】各サイトのポイント単位について:
サイトによってポイント⇔円の交換レートが異なります(下記)。valueには円換算した
金額ではなく、各サイトのページに書かれている生の数値(例: "10,000pt"なら10000、
%還元なら%の数値そのもの)をそのまま入れてください。円換算はこちらのアプリ側で
別途行うため、あなたが単位変換をする必要はありません(変換すると二重変換の原因に
なります)。比較ブログが円換算後の金額しか載せていない場合は、そのサイトの交換
レートで割り戻してから生のポイント数に変換してください。
