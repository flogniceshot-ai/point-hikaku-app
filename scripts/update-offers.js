...
// サイトごとのポイント⇔円交換レート(1ポイント=何円か)。
// 例: ECナビ・アメフリ・ポイントインカムは10pt=1円(0.1)、Powlは約10.5pt=1円。
// DB(campaign_offers.reward_value)には必ず「そのサイト上で実際に表示されている
// 生のポイント数(または%)」を保存する。円換算はアプリ側(point_sites.point_rate)で
// 都度計算するため、ここで円に変換した数値を保存してはいけない。
const SITE_POINT_RATE_NOTES = {
  "ハピタス": "1pt=1円",
  "モッピー": "1P=1円",
  "ちょびリッチ": "1pt=1円",
  "Powl": "約10.5pt=1円(1円未満はptのまま表示される)",
  "ポイントインカム": "10pt=1円",
  "コインカム": "1CIM=1円",
  "ワラウ": "1pt=1円",
  "ECナビ": "10pt=1円",
...
function buildPrompt(keyword) {
  ...
  const rateNotes = TARGET_SITES.map((s) => `  - ${s}: ${SITE_POINT_RATE_NOTES[s]}`).join("\n");
  return `あなたはポイ活比較サイトのデータ調査アシスタントです。
今日の日付は${today}です。
「${keyword}」という案件について、Web検索を使って調べ、以下のポイントサイトそれぞれの
「本日${today}時点」で、そのサイト上に実際に表示されている還元数値をそのまま教えてください。

対象サイト: ${TARGET_SITES.join("、")}

【重要】各サイトのポイント単位について:
サイトによってポイント⇔円の交換レートが異なります(下記)。
value には円に換算した金額ではなく、各サイトのページに書かれている
「生の数値」(例: ECナビで"10,000pt"と表示されていれば10000、%還元なら%の数値そのもの)を
そのまま入れてください。円換算はこちらのアプリ側で別途行うため、
あなたが円換算・単位変換をする必要はありません(むしろ変換すると二重変換の原因になります)。
...
- 比較ブログが円換算後の金額しか載せていない場合は、そのブログが前提としている
  交換レートで割り戻し、各サイトの生のポイント数に変換してから value に入れてください
  (例: ブログに「ECナビ経由で1,000円」とあれば、ECナビは10pt=1円なのでvalueは10000)。
