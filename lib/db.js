...
// ------------------------------------------------------------------
// 新しいポイントサイトを追加(管理画面用)。
// 手動取り込みAPI(upsertOffersFromImport)は、point_sitesに存在しないslugを
// 「未知のサイトです」として拒否するため、案件データを取り込む前にまずここで
// サイト自体を登録しておく必要がある。既に同じslugがあれば内容を更新するだけにする。
// ------------------------------------------------------------------
export async function addSite({ slug, name, siteUrl, pointRate, colorHex }) {
  const p = getPool();

  if (!p) {
    const existing = seed.sites.find((s) => s.slug === slug);
    if (existing) {
      existing.name = name;
      existing.colorHex = colorHex || existing.colorHex;
      existing.pointRate = pointRate ?? existing.pointRate;
    } else {
      seed.sites.push({ slug, name, colorHex: colorHex || null, pointRate: pointRate ?? 1, affiliateUrl: null });
    }
    persistSeed();
    return true;
  }
  ...
  await p.query(
    `INSERT INTO point_sites (slug, name, site_url, point_rate, color_hex, is_active)
     ...
     ON CONFLICT (slug) DO UPDATE SET
       name = EXCLUDED.name,
       site_url = COALESCE(EXCLUDED.site_url, point_sites.site_url),
       point_rate = COALESCE(EXCLUDED.point_rate, point_sites.point_rate),
       color_hex = COALESCE(EXCLUDED.color_hex, point_sites.color_hex),
       is_active = true`,
    [slug, name, siteUrl || null, pointRate ?? 1, colorHex || null]
  );
  return true;
}
...
function formatCampaignMock(c) {
  ...
  const offers = c.offers
    .map((o) => {
      ...
      return {
        site: site?.name || o.siteSlug,
        colorHex: site?.colorHex,
        pointRate: site?.pointRate ?? 1,
