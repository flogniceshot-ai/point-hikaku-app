...
// seed.json側のoffers[].valueは常に「サイト上の生のポイント数(または%)」で保持する
// (円換算はアプリ側のpointRateで別途行うため)。
...
「そのサイト上に実際に表示されている還元数値」をそのまま教えてください。
...
%還元なら%の数値そのもの)をそのまま入れてください。円換算はアプリ側で別途行うため、
あなたが単位変換をする必要はありません(変換すると二重変換の原因になります)。
比較ブログが円換算後の金額しか載せていない場合は、そのサイトの交換レートで
割り戻してから生のポイント数に変換してください。
