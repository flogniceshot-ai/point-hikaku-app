...
export async function upsertOffersFromImport({ siteSlug, entries }) {
  ...
  for (const e of entries) {
    ...
    await p.query(
      `INSERT INTO campaign_offers (campaign_id, site_id, reward_value, is_first_time_only, is_guaranteed, fetched_at)
       ...
       ON CONFLICT (campaign_id, site_id) DO UPDATE SET
         ...
         fetched_at = now()`,
      [campaignId, siteId, e.value, !!e.firstTimeOnly, !!e.guaranteed]
    );
