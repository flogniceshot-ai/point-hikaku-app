...
export async function searchCampaigns(keyword) {
  ...
  const { rows } = await p.query(
    `SELECT c.id, c.canonical_name, c.reward_type,
            ...
            s.slug AS site_slug, s.name AS site_name, s.color_hex
     FROM campaigns c
     ...
     ORDER BY c.canonical_name, o.reward_value DESC`,
    [keyword || ""]
  );
...
export async function getCampaignById(id) {
  ...
  const { rows: offerRows } = await p.query(
    `SELECT o.reward_value, o.is_guaranteed, o.is_first_time_only, o.offer_url, o.fetched_at, s.slug, s.name, s.color_hex
